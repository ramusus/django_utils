...
def thumbnail(image_path, width=None, height=None):
    ...
    try:
        ...
        image = get_thumbnail(image_path, '%sx%s' % (width, height))
    ...
    return u'<img src="%s%s" alt="%s" />' % (settings.MEDIA_URL, image_url, image_path)
    ...
    def render(self, name, value, attrs=None):
        ...
        if file_name:
            ...
            try:  # is image
                ...
                output.append(
                    '<a target="_blank" href="%s">%s</a><br />%s <a target="_blank" href="%s">%s</a><br />%s ' %
                    (file_path, thumbnail(file_name, self.width, self.height), _('Currently:'), file_path, file_name,
                     _('Change:')))
