...
class LinkFieldWidget(Widget):
    ...
    def render(self, name, value, attrs=None):
        ...
        output.append('''<input type="text" name="%s" id="id_%s" value="%s" onkeyup="this.value = this.value.replace(/^%s/i, '$1'); $('a#%s-link').attr('href', '%s'.replace('%s', this.value)).css({'display': (this.value ? 'inline' : 'none')})" />''' % (name, name, value or '', replace_pattern, name, self.url, '%s'))
